...
volatile uint8_t humidity_high, humidity_low, temperature_high, temperature_low;
  ...
  while (1)
  {
		  ...
		  if (status == 1)
		  {
			  ...
			  if(temp2>15 && temp2<=23)                        				//Ambient Temperature Condition
				  HAL_GPIO_TogglePin(led_green_GPIO_Port, led_green_Pin);   //green led for ambient temperature
			  if(temp2>23)
				  HAL_GPIO_TogglePin(led_red_GPIO_Port, led_red_Pin);       //red led when termperature >t hrehold
